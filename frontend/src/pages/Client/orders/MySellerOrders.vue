...
// Mặc định từ ngày 01/10 của năm hiện tại
const getSpecificStartDate = () => {
    const date = new Date();
    // Tháng 10 là index 9 trong JS Date
...
// 2. Định nghĩa State (Filters)
const filters = ref({
    status: '',
    search: '',
    from_date: getSpecificStartDate(),
...
function handleFilter() {
    filters.value.page = 1; // Reset về trang 1 khi bấm Lọc
    fetchData();
        ...
        <div class="bg-gray-50 min-h-screen pb-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <!-- Phân trang -->
                    <div v-if="pagination && (pagination.lastPage >= 1 || orders.length > 0)" class="bg-white px-4 py-3 border-t border-gray-200 flex items-center justify-between sm:px-6">
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col animate__animated animate__fadeInUp">
                ...
                <div class="p-4 border-t bg-gray-50 flex justify-end space-x-3" v-if="currentOrder">
                    ...
                    <template v-if="currentOrder.status === 'pending'">
                        ...
                        <button @click="handleApprove(currentOrder)" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium shadow-sm">Duyệt đơn hàng</button>
                    </template>
